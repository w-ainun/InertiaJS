...
import { Building, ChevronLeft, Clock, Globe, Home, Info, LoaderCircle, Mail, MapPin, Navigation, Save, Search, User, Users } from 'lucide-react';
...
export default function AddressesEdit() {
  const { address, contacts, success, error } = usePage<
    SharedData & {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
      title: 'Edit',
      href: route('address.edit', address.data.id),
    },
  ];
  ...
  // Get contacts data - check if contacts exists and has data
  const contactsData = contacts?.data || [];

  // Make sure the current contact is included in the options
  ...
  const contactOptions = allContacts.map((contact) => ({
    ...
    avatar: contact.profile && !contact.profile.startsWith('http') ? `/storage/${contact.profile}` : contact.profile,
  }));

  // Get selected contact info - either from form data or current address contact
  const selectedContact = allContacts.find((contact) => String(contact.id) === String(data.contact_id)) || address.data.contact;
      ...
      <div className="container px-4 py-8">
        <Card className="border shadow-sm">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <CardDescription className="mt-1">Update address information for {address.data.contact?.name || 'this contact'}</CardDescription>
              </div>
          ...
          <div className="bg-muted/20 border-b px-6 py-4">
            <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono font-medium">{address.data.post_code}</span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="text-muted-foreground h-4 w-4" />
                <span className="text-muted-foreground">Last Updated:</span>
                <span className="font-medium">
                  {address.data.updated_at ? formatDistanceToNow(new Date(address.data.updated_at), { addSuffix: true }) : 'Never'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-muted/10 border-b px-6 py-4">
              <div className="flex items-center gap-4">
                <Avatar className="border-border h-12 w-12 border-2">
                  <AvatarImage
                    src={
                      selectedContact.profile && selectedContact.profile.startsWith('http')
                        ? selectedContact.profile
                        : selectedContact.profile
                          ? `/storage/${selectedContact.profile}`
                          : '/placeholder.svg'
                    }
                ...
                <Badge variant="outline">Address owner</Badge>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
              ...
              <CardContent className="p-6">
                <TabsContent value="contact" className="mt-4 space-y-6">
                  ...
                  >
                    ...
                    <AlertDescription>
                      Update the contact associated with this address. You can search by name, phone, or owner information.
                    </AlertDescription>
                  </Alert>
                ...
                <TabsContent value="street" className="mt-4 space-y-6">
                  ...
                  >
                    ...
                    <AlertDescription>
                      Update the detailed street address including house number, street name, and any additional information like RT/RW, building
                      name, or landmarks.
                    </AlertDescription>
                  </Alert>
                ...
                <TabsContent value="location" className="mt-4 space-y-6">
                  ...
                  >
                    ...
                    <AlertDescription>
                      Update the city, province, country, and postal code for this address. This information is used for mapping and delivery
                      purposes.
                    </AlertDescription>
                  </Alert>
            ...
            <div className="bg-muted/30 flex items-center justify-between p-6">
