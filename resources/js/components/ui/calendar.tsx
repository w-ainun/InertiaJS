...
function Calendar({
  ...
  const { onNextClick, onPrevClick, startMonth, endMonth, selected } = props
...
function Nav({
  ...
  const isPreviousDisabled = (() => {
    ...
    if (navView === "months") {
      ...
      return (
        (startMonth && differenceInCalendarDays(new Date(currentYear - 1, 11, 31), startMonth) < 0) ||
        (endMonth && differenceInCalendarDays(new Date(currentYear - 1, 0, 1), endMonth) > 0)
      )
    }
  ...
  const isNextDisabled = (() => {
    ...
    if (navView === "months") {
      const currentYear = (selected as Date | undefined)?.getFullYear() ?? new Date().getFullYear()
      return (
        (startMonth && differenceInCalendarDays(new Date(currentYear + 1, 0, 1), startMonth) < 0) ||
        (endMonth && differenceInCalendarDays(new Date(currentYear + 1, 11, 31), endMonth) > 0)
      )
    }
    return !nextMonth
  })()
  ...
  const handlePreviousClick = React.useCallback(() => {
    ...
    if (navView === "months") {
      ...
      goToMonth(new Date(newYear, (selected as Date | undefined)?.getMonth() ?? 0))
      onPrevClick?.(new Date(newYear, 0, 1))
      return
    }
    if (!previousMonth) return
    goToMonth(previousMonth)
    onPrevClick?.(previousMonth)
  }, [navView, previousMonth, goToMonth, onPrevClick, setDisplayYears, displayYears.from, displayYears.to, selected])
  ...
  const handleNextClick = React.useCallback(() => {
    ...
    if (navView === "months") {
      ...
      const newYear = currentYear + 1
      goToMonth(new Date(newYear, (selected as Date | undefined)?.getMonth() ?? 0))
      onNextClick?.(new Date(newYear, 0, 1))
      return
    }
    if (!nextMonth) return
    goToMonth(nextMonth)
    onNextClick?.(nextMonth)
  }, [navView, nextMonth, goToMonth, onNextClick, setDisplayYears, displayYears.from, displayYears.to, selected])
  return (
...
function CaptionLabel({
  ...
  // Days view - show month and year as separate clickable elements
  const childrenString = children?.toString() || ""
  const parts = childrenString.split(" ")
  const monthName = parts[0] || ""
  const year = parts[1] || ""

  return (
    <div className="flex items-center gap-1 h-7 w-full justify-center">
      <Button
...
function MonthSelectGrid({
  ...
  const { goToMonth, selected } = useDayPicker()
  const currentYear = new Date().getFullYear()
  const selectedYear = (selected as Date | undefined)?.getFullYear() ?? currentYear
  ...
  return (
    <div className={cn("grid grid-cols-3 gap-y-2", className)} {...props}>
      {months.map((month, index) => {
        ...
        const isBefore = startMonth && differenceInCalendarDays(new Date(selectedYear, index, 31), startMonth) < 0
        const isAfter = endMonth && differenceInCalendarDays(new Date(selectedYear, index, 0), endMonth) > 0
        const isDisabled = isBefore || isAfter
